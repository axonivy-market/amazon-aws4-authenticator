...
class TestAws4AuthenticatorFeature {
  ...
  private Object getSecretKey() {
    var value = System.getenv("AWS_SECRET_KEY");
    assertThat(value)
        .as("AWS secret key must be provided using system property 'AWS_SECRET_KEY'")
        .isNotBlank();
    return value;
  }

  private Object getAccessKey() {
    var value = System.getenv("AWS_ACCESS_KEY");
    assertThat(value)
        .as("AWS access key must be provided using system property 'AWS_ACCESS_KEY'")
        .isNotBlank();
    return value;
  }
}
